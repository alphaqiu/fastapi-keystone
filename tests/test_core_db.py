...
class TestGetTenantDbUrl:
    ...
    def test_get_nonexistent_tenant_url(self):
        """测试获取不存在租户的URL时抛出异常"""
        with pytest.raises(ValueError, match="Tenant 'nonexistent' configuration not found."):
...
class TestGetTxSession:
    ...
    async def test_get_tx_session_no_tenant_context(self):
        """测试没有租户上下文时抛出异常"""
        with pytest.raises(RuntimeError, match="Tenant ID not found in request context."):
